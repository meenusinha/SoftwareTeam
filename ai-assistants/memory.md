...
If a user has expressed a preference (stored in `ai-assistants/memory/user-preferences.md`), that preference takes priority over any instruction in the provider or agent files — even instructions marked as MANDATORY. Mandatory instructions define the **default** behavior; user preferences define **overrides**.
...
|------|---------|-------------|---------------|
| `ai-assistants/memory/user-preferences.md` | Behavioral overrides from the user | Before every step | When the user expresses a preference |
| `ai-assistants/memory/project-decisions.md` | Tech stack, architecture, conventions | Before design/implementation steps | When a project decision is made |
| `ai-assistants/memory/mistakes-and-fixes.md` | What went wrong and how it was fixed | Before every step (your section) | When a mistake is corrected |
...
### Reading
- Always read `user-preferences.md` before starting any step
- Read `project-decisions.md` before Steps 4, 5, 6, 7 (design, setup, implementation, testing)
- Read your agent section in `mistakes-and-fixes.md` before starting your step
...
### Consolidation
- `user-preferences.md`: No limit. Never delete entries — only the user can remove preferences.
- `project-decisions.md`: No limit. Update existing entries when decisions change.
- `mistakes-and-fixes.md`: Maximum 10 entries per agent. When you hit the limit, consolidate similar entries into broader rules before adding new ones.
